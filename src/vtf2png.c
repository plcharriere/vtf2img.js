...
void decode_dxt5(vtf_header_t* header, uint8_t *image_end, int frame_offset, uint8_t** rgba_rows)
{
  int framesize = ((header->width+3)/4) * ((header->height+3)/4) * (128/8);
  uint8_t *data = image_end - (framesize*frame_offset);
  int pos = 0;
...
unsigned short vtf2width(uint8_t* filedata) {
  vtf_header_t* header = (vtf_header_t*)filedata;

  return header->width;
}

unsigned short vtf2height(uint8_t* filedata) {
  vtf_header_t* header = (vtf_header_t*)filedata;

  return header->height;
}

unsigned short vtf2frames(uint8_t* filedata) {
  vtf_header_t* header = (vtf_header_t*)filedata;

  return header->frames;
}

uint8_t* vtf2rgb(uint8_t* filedata, int filesize, int frame) {
    vtf_header_t* header = (vtf_header_t*)filedata;
    ...
    uint8_t array[header->width * header->height * 4];
    int k = 0;
    for(int i = 0; i < header->height; ++i) {
        for (int j = 0; j < header->width * 4; ++j) {
            array[k++] = rgba_rows[i][j];
        }
    }
